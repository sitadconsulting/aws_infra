...
variable "dx_private_virtual_interface_name" {
  description = "The name for the virtual interface"
  type        = string
}
variable "dx_private_virtual_interface_sitelink_enabled" {
  description = "Indicates whether to enable or disable SiteLink"
  type        = bool
  default     = false
}
variable "dx_private_virtual_interface_tags" {
  description = "A map of tags to assign to the resource. If configured with a provider default_tags configuration block present, tags with matching keys will overwrite those defined at the provider-level"
  type        = map(string)
  default     = {}
}
variable "dx_private_virtual_interface_vlan" {
  description = "The VLAN ID"
  type        = number
}
variable "dx_private_virtual_interface_vpn_gateway_id" {
